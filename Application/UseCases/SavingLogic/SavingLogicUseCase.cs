...
namespace Application.UseCases.SavingLogic
{
    ...
    public class SavingLogicUseCase : ISavingLogicUseCase
    {
        ...
        public SavingLogicResult Save(SavingLogicRequest request)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(request.TextContent) && (request.WasChanged || request.NoteModeChanged))
                {
                    ...
                    var prevFile = string.IsNullOrEmpty(request.Filename) ? prevTextName
                                                                          : request.Filename;
            ...
            {
                return new SavingLogicResult(false, 
                                            "Критическая ошибка при сохранении: "+ex.Message, 
                                            request.FirstFileIndex, 
                                            request.ButtonResaveThisChecked, 
                                            request.Filename, 
                                            false, 
                                            request.NoteModeChanged);
            }
        }
    }
}
